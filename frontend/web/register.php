    ...
    <div class="w-full max-w-md relative z-10">
        <div class="neumorph p-9 register-card">
            ...
            <div class="text-center mb-8">
                ...
                <h1 class="text-3xl font-bold mb-1" data-i18n="register-title">Criar Conta</h1>
            ...
            <form id="registerForm" method="post"   action="<?= url("register/") ?>"    class="space-y-5">
                <input type="text" name="name"  id="name" placeholder="Nome completo" required class="neu-input">
                <input type="email" name="email"  id="email" placeholder="Email" required class="neu-input">
                <input type="tel" name="phone"  id="phone" placeholder="Telefone (+244)" required class="neu-input">

                <div class="grid grid-cols-2 gap-4">
                    <select id="role" name="role"  class="neu-input">
                        <?php foreach(role() as $f=>$r): ?>
                              <option value="<?=$f ?>"><?= $r ?></option>
                        <?php endforeach; ?>
                    </select>
                    <select id="gender" name="gender"  class="neu-input">
                        <option value="">Género</option>
                        <?php foreach(gender() as $f=>$r): ?>
                              <option value="<?=$f ?>"><?= $r ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <input type="password" name="confirm_password"  id="confirm_password" placeholder="Confirmar palavra-passe" required class="neu-input">

                <div class="flex items-center gap-2 text-sm">
                    <input type="checkbox" id="terms" required class="accent-[var(--accent)] w-4 h-4">
                    <label for="terms" class="text-xs text-[var(--muted)] cursor-pointer">
                        Concordo com os <span class="text-[var(--blue)] hover:underline">Termos</span>
                    </label>
                </div>

                <button type="submit"  name="register-button"  class="btn-primary w-full text-base">
                    <span data-i18n="register-button">Criar Minha Conta</span>
                </button>
            </form>

            <div class="text-center mt-6 text-sm">
                <span class="text-[var(--muted)]">Já tem conta?</span>
                <a href="<?= url("/login") ?>" class="ml-1 font-semibold text-[var(--accent)]">Entrar</a>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        const translations = {
            pt: {
                ...
                "register-button": "Criar Minha Conta"
            },
            en: {
